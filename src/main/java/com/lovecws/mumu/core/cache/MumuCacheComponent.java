...
package com.lovecws.mumu.core.cache;

import com.lovecws.mumu.core.utils.SerializeUtils;
    ...
    public Object handler(ProceedingJoinPoint joinPoint, MumuCache modularCacheAnnotation, Object[] args) throws Throwable {
        ...
        if (modularCacheAnnotation.expire() == 0) {
            ...
            if (cacheKeyBytes == null) {
                proceed = joinPoint.proceed();
                cache(cacheKey, SerializeUtils.serialize(proceed), modularCacheAnnotation.expire());
            }
            //缓存不为空 则获取缓存对象
            else {
                try {
                    proceed = SerializeUtils.deserialize(cacheKeyBytes);
